                        ...
                        items(taskList) { task ->
                            SwipeableTaskCard(
                                ...
                                onSkip = {
                                    skipTask(db, task)
                                    taskList = taskList.filter { it.id != task.id }
                                    navController.navigate("skipped")
                                }
                            )
                            Spacer(modifier = Modifier.height(4.dp))
                        }
...
// Firestore İşlevleri
fun completeTask(db: FirebaseFirestore, task: Task) {
...
fun skipTask(db: FirebaseFirestore, task: Task) {
    val skippedTask = task.copy(skipped = true)
    val userId = FirebaseAuth.getInstance().currentUser?.uid ?: return
    db.collection("profiles")
        .document(userId)
        .collection("tasks")
...
fun deleteTask(db: FirebaseFirestore, task: Task) {
    val userId = FirebaseAuth.getInstance().currentUser?.uid ?: return
    db.collection("profiles")
        .document(userId)
        .collection("tasks")
...
suspend fun getNormalTasks(db: FirebaseFirestore): List<Task> {
    val userId = FirebaseAuth.getInstance().currentUser?.uid ?: return emptyList()
    return db.collection("profiles")
        .document(userId)
        .collection("tasks")
        .whereEqualTo("completed", false)
        .whereEqualTo("skipped", false)
